...
export function generateStaticParams() {
  return locales.flatMap((lang) => getProjects(lang).map((project) => ({ lang, slug: project.slug })));
}
...
export default async function ProjectPage({ params }: PageProps<"/[lang]/projects/[slug]">) {
  ...
  const { common, categories } = getDictionary(lang);
    ...
    <>
      ...
      <main className="lg:grid lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        <aside className="bg-sand px-5 pt-8 pb-12 lg:sticky lg:top-(--nav-height) lg:h-[calc(100svh-var(--nav-height))] lg:overflow-y-auto lg:px-10">
          <Link href={`/${lang}/projects`} className="group caption inline-flex items-center gap-3 border border-ink p-1 pr-3">
          ...
          />
          {!project.translated && (
            <p lang={lang} className="caption mt-8 text-muted">
              {common.romanianOnly}
            </p>
          )}
          <div
            lang={project.translated ? lang : defaultLocale}
      ...
      <Link href={`/${lang}/projects/${next.slug}`} className="group mt-3 flex items-end justify-between border-y border-line px-3 py-10">
        <span>
